...
void uart_read_charList(char* read_data,int data_len)
{
    ...
    len = UART0_Recv(fd, read_data,data_len);
    if(len > 0)
    {
        //rcv_buf[len] = '\0';
        printf("receive data is\n");
...
void uart_read_gyro(int fd,char* read_data)
{
    //int fd;                            //文件描述符
    int err;                           //返回调用函数的状态
    int len;
    ...
    while(counter!=11)
    {
        len = UART0_Recv(fd, read_data+counter,1);//read a byte
